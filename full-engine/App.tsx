...
interface DlItem {
  id: string;        // download id (Rust control key)
...
function App() {
  ...
  const dlSpeedRef = useRef<Map<string, { t: number; b: number }>>(new Map());
  ...
  // Kick off a set of downloads IN PARALLEL. `group` (with a folder name) makes them
  // offer an "import as folder?" prompt once every part finishes.
  const startDownloads = useCallback(async (
    items: { url: string; name: string; size?: number }[],
    dir: string,
    group?: { id: string; name: string },
  ) => {
    const { invoke } = await import('@tauri-apps/api/core');
    const { join } = await import('@tauri-apps/api/path');
    const fresh = items.filter(it => !videoLibrary.some(v => v.name === it.name));
    ...
    const newItems: DlItem[] = [];
    for (const it of fresh) {
      const id = 'dl-' + genId();
      const libId = genId();
      const dest = await join(dir, it.name);
      dlSpeedRef.current.set(id, { t: Date.now(), b: 0 });
      newItems.push({ id, libId, name: it.name, url: it.url, dest, bytes: 0, total: it.size || 0, speed: 0, status: 'downloading', group: group?.id });
      // Fire the transfer — all at once, no awaiting between them (parallel).
      invoke('download_file', { url: it.url, dest, id }).catch(() => {
  ...
  // "Watch now": stream the first item immediately, download ALL in parallel. When
  // the watched item's download finishes we swap it to the local file at the same
  // second (same libId → the resume-progress restore seeks back).
  ...
  // Single global listener drives EVERY parallel download: live bytes + speed, and
  // on completion → library import, folder-group prompt, and stream→local hand-off.
  useEffect(() => {
    let un = () => {};
    (async () => {
      const { listen } = await import('@tauri-apps/api/event');
      const { convertFileSrc } = await import('@tauri-apps/api/core');
      un = await listen<{ id: string; transferred: number; total: number; done: boolean }>('share-progress', async (e) => {
        const item = downloadsRef.current.find(d => d.id === e.payload.id);
        if (!item) return;

        if (!e.payload.done) {
          // Per-item speed sample (~2/sec) — real throughput, not a constant.
          const s = dlSpeedRef.current.get(item.id) ?? { t: Date.now(), b: 0 };
          const now = Date.now();
          const dt = (now - s.t) / 1000;
          let speed = item.speed;
          if (dt >= 0.5) { speed = Math.max(0, (e.payload.transferred - s.b) / dt); dlSpeedRef.current.set(item.id, { t: now, b: e.payload.transferred }); }
          setDownloads(prev => prev.map(d => (d.id === item.id ? { ...d, bytes: e.payload.transferred, total: e.payload.total || d.total, speed, status: 'downloading' } : d)));
          return;
        }

        // Completed → add to library (thumbnail + size), mark done, hand off if watching.
        const meta: VideoMeta = { id: item.libId, name: item.name, path: item.dest, size: 0, addedAt: Date.now(), type: typeFromPath(item.name) };
        try { await videoStore.save({ ...meta }); } catch {}
        setVideoLibrary(prev => prev.some(v => v.name === item.name) ? prev : [meta, ...prev]);
        enqueueMetaExtraction([{ path: item.dest, id: item.libId }]);
        dlSpeedRef.current.delete(item.id);
        setDownloads(prev => prev.map(d => (d.id === item.id ? { ...d, bytes: d.total || d.bytes, status: 'done' } : d)));
        // Hand off a currently-streaming copy to the finished local file.
        setPlaylist(prev => prev.map(it => (it.path === item.url ? { ...it, src: convertFileSrc(item.dest), path: item.dest, id: item.libId } : it)));

        // Folder group: once every part is in, offer to import them as a folder.
        if (item.group) {
          const g = dlGroupsRef.current.get(item.group);
          if (g) {
            g.libIds.push(item.libId);
            if (g.libIds.length >= g.total) {
              const ids = [...g.libIds]; const gname = g.name;
              dlGroupsRef.current.delete(item.group);
              showToast(`Downloaded “${gname}” (${ids.length} files)`, {
  ...
  // Download controls (per item).
  const pauseDownload = useCallback(async (id: string) => {
    const { invoke } = await import('@tauri-apps/api/core');
    await invoke('download_control', { id, action: 'pause' }).catch(() => {});
  ...
  const resumeDownload = useCallback(async (id: string) => {
    ...
    if (!item) return;
    const { invoke } = await import('@tauri-apps/api/core');
    dlSpeedRef.current.set(id, { t: Date.now(), b: item.bytes });
    setDownloads(prev => prev.map(d => (d.id === id ? { ...d, status: 'downloading' } : d)));
    invoke('download_file', { url: item.url, dest: item.dest, id }).catch(() => {
      setDownloads(prev => prev.map(d => (d.id === id ? { ...d, status: 'error' } : d)));
    });
  }, []);
  const cancelDownload = useCallback(async (id: string) => {
    const item = downloadsRef.current.find(d => d.id === id);
    const { invoke } = await import('@tauri-apps/api/core');
    if (item && item.status === 'downloading') await invoke('download_control', { id, action: 'cancel' }).catch(() => {});
    dlSpeedRef.current.delete(id);
  ...
  return (
    <div className={`w-screen h-screen ${playlist.length > 0 ? 'bg-transparent' : 'bg-neutral-900'} text-white overflow-hidden flex flex-col font-sans`}>
          ...
          <div className="relative flex-1 h-full bg-transparent">
            <VideoPlayer
              ...
              downloadProgress={(() => {
                ...
                const d = cur && downloads.find(x => x.status !== 'done' && (x.url === cur.path || x.libId === cur.id));
      ...
      <ShareModal
        ...
        onWatchAndDownload={watchAndDownload}
