...
export function encodeShareLink(p: SharePayload): string {
  const kind = (p.k === 'folder' || p.k === 'lan-folder') ? 'folder' : 'video';
...
/** Stream-download (resumable) to `destPath`. Returns "done" | "paused" | "cancelled". */
export async function downloadItem(url: string, destPath: string, id: string): Promise<string> {
  return invoke<string>('download_file', { url, dest: destPath, id });
}

/** Pause (keeps partial → resume later) or cancel (deletes partial) a download. */
export function controlDownload(id: string, action: 'pause' | 'cancel'): Promise<void> {
